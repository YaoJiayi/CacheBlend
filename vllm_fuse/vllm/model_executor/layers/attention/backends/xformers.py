...
class XFormersBackend:
    ...
    def forward(
        ...
        # Do checking here
        if status in [1]:
            #if seq_len>4000 and input_metadata.is_prompt:
            ...
            #HACK(Jiayi): make_70b should be used for all gqa
        ...
        #if seq_len>3800 and input_metadata.is_prompt:
        #    torch.cuda.synchronize()
        #    start = torch.cuda.Event(enable_timing=True)
        #    end = torch.cuda.Event(enable_timing=True)
        #    start.record()
            ...
            if (key_cache is None or value_cache is None
                ...
                if input_metadata.attn_bias is None:
                    if self.alibi_slopes is None:
                        ...
                        #        self.sliding_window)
                        #HACK(Jiayi): force
                        #input_metadata.attn_bias = _fetch_maetrailized_mask(query.shape[0],self.num_kv_heads,query.device,query.dtype)
                        #HACK(Jiayi): fetch_70b should be used for all gqa 
                ...
                #    torch.cuda.synchronize()
                #    temp_time = start.elapsed_time(end)
                #    print(f"xops attn time:{temp_time}")
...
def _fetch_maetrailized_mask(q_len,num_head,device,dtype):
    seq_len = q_len
    padded_len = (seq_len + 7) // 8 * 8
