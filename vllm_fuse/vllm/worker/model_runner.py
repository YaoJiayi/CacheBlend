...
class ModelRunner:
    ...
    def execute_model(
        ...
        print(f"input shape(model_runner.py): {input_tokens.shape}")
        # Execute the model.
        if input_metadata.use_cuda_graph:
            ...
            hidden_states, _ = model_executable(
            ...
            print(f"cuda graph time: {temp_time}")
        else:
        ...
        if sampling_metadata.perform_sampling==True:
            if input_tokens.shape[1]>900: #and torch.distributed.get_rank()==0:#self.model.model.cache_fuse_metadata["check"]:
